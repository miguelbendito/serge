...
from sqlalchemy import Integer, String, Text, Boolean,asc, desc
...
class Menu(db.Model):
    __tablename__ = "menus"
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    season: Mapped[str] = mapped_column(String(250), nullable=True)
...
def admin_menus():
    ...
    if sort == "title_asc":
        query = query.order_by(asc(Menu.title))
    elif sort == "title_desc":
        query = query.order_by(desc(Menu.title))
    elif sort == "season_asc":
        query = query.order_by(asc(Menu.season))
    elif sort == "season_desc":
        query = query.order_by(desc(Menu.season))
    elif sort == "status":
        query = query.order_by(desc(Menu.is_active))

    menus = db.session.execute(query).scalars().all()
...
def add_new_menu():
    ...
    if form.validate_on_submit():
        if form.img_file.data:
            ...
            menu_img_url = result["secure_url"]
        else:
            menu_img_url = None
...
@app.route("/admin/menu/<int:menu_id>/toggle_status", methods=["POST"])
@admin_only
